...
## 2026-05-09 — Memory id is Supermemory's `customId`
**Choice:** `Memory.id` (the local primary key) is passed to Supermemory as `customId` on every `client.add` call. The locally-generated UUID is the cross-system handle; Supermemory's internal `document_id` is treated as opaque (kept in `external_id` for the patch/delete path).
**Why:** Lets a retried `create_memory` carry the same handle to both systems, so a partial failure followed by a retry converges on a single document instead of two. The same UUID also doubles as the layer-1 idempotency key (`create_memory(idempotency_id=...)` stores it as `id`). Two roles, one identifier — fewer moving parts.
**Alternatives considered:** Generate a separate `customId` distinct from the row's PK (rejected — extra column, extra retry-handle plumbing for no benefit at single-user scale); rely on Supermemory's `document_id` exclusively (rejected — not known until after the inline call, can't be quoted in a retry).

## 2026-05-09 — Local Postgres is source of truth; Supermemory is a derived index
**Choice:** Every memory write commits to Postgres first; the Supermemory call is best-effort within the same request. Reads (list, detail) go to Postgres exclusively. Search prefers Supermemory but falls back to local Postgres FTS on any client error.
**Why:** Postgres is durable, transactional, and under our operational control; Supermemory is an external dependency reached over a network. Inverting that — treating Supermemory as primary — would couple availability of basic read/write to an external service and force complex reconciliation on partial failure. With Postgres as the source of truth, an outage at Supermemory only degrades search ranking quality, not capture/edit/browse.
**Alternatives considered:** Treat Supermemory as primary and reconcile locally (rejected for the availability reason); dual-write with consensus (rejected — operationally heavy for a single-user app).

## 2026-05-09 — Both dedupe layers in `create_memory` are silent on hit
**Choice:** Layer-1 (idempotency UUID) and layer-2 (canonical content hash) dedupe in `create_memory` both return the existing row WITHOUT raising. Only `update_memory`'s `IntegrityError → MemoryDuplicate` path returns a 409. As a result, the HTTP create endpoint returns `201` for both fresh inserts and dedupe hits.
**Why:** Idempotency exists so callers can retry safely. A 409 on retry would force every caller to special-case "I already created this" — exactly the boilerplate idempotency was meant to remove. The collision-error semantics belong on `update_memory`, where two distinct rows actually want the same canonical text and the user has to pick one.
**Alternatives considered:** Return `200` on dedupe hits and `201` on fresh inserts (rejected — caller would have to branch on status code to know whether their request "did something"; both responses describe the same observable end state); raise `MemoryDuplicate` on layer-2 hit (rejected for the retry-boilerplate reason above).

## 2026-05-09 — Soft-delete local, hard-delete remote; tombstones never purged in v1
**Choice:** `delete_memory` sets `Memory.deleted_at` (preserving the row) and issues `client.delete` against Supermemory (removing the document). On a remote-side failure the row is marked `external_status='pending_delete'` and the local soft-delete still applies. There is no v1 process that purges tombstones — they accumulate.
**Why:** The local tombstone enables idempotent re-deletes, supports the partial-unique index that excludes deleted rows from the content-hash check, and gives the user a future "trash bin" feature without changing the schema. Hard-deleting remotely keeps Supermemory's index lean (search results never include rows the user has deleted). Skipping the purge job in v1 trades a small amount of disk for one less moving part — for a single-user journal, "ever-growing tombstones" is years away from being a problem.
**Alternatives considered:** Hard-delete locally too (rejected — loses the partial-unique-index trick and the trash-bin option); soft-delete on both sides (rejected — Supermemory search would return tombstoned hits we'd then have to filter); add a tombstone GC job in v1 (rejected — premature for the data volumes involved).

## 2026-05-09 — Sync is inline + per-memory manual retry; no cron, no daemon, no outbox
**Choice:** Memory writes call Supermemory inline within the request handler (≤2s timeout). On failure the row is stored locally with `external_status='unsynced'`; the user retries one row at a time via `POST /memories/{id}/sync`. There is no background sweeper, no cron, no daemon, no outbox queue.
**Why:** The outbox/worker pattern is the orthodox answer for async write reliability — but it brings a process to run, a queue to monitor, retry-budget tuning, and a poison-message strategy. For a single-user app where the user is right there with a retry button, the marginal availability gain doesn't justify the operational surface. If usage patterns ever change (multi-user, high write volume), reintroducing this is a net-new phase.
**Alternatives considered:** Outbox + worker (rejected — see above); periodic background re-sync of all unsynced rows (rejected — same operational cost, less observable); fire-and-forget with no retry surface (rejected — no path to recover from a transient blip).

## 2026-05-09 — Container tags are `user_<owner_uuid_no_dashes>` only (no `kind_*` in v1)
**Choice:** Every Supermemory write tags the document with exactly one container: `user_<owner_uuid.hex>`. There are no per-category tags (no `kind_journal`, `kind_idea`, etc.) in v1.
**Why:** The single user-scoped tag is what makes search filterable to one user's data. Adding category taxonomies before knowing how the user actually slices their entries would lock in vocabulary that may not match their real mental model — easier to add tags later than to migrate away from premature ones. Single-tag also minimises coupling between schema decisions and the chat agent's prompt design.
**Alternatives considered:** Pre-define `kind_*` tags now (rejected — speculative; LLM-side classification would need a stable taxonomy that doesn't yet exist); store free-form tags as another column (rejected — out of scope for capture; revisit when the agent has a reason to write them).

## 2026-05-09 — Step-up DELETE returns 401 (not 403)
**Choice:** When `DELETE /memories/{id}` is called without (or with a wrong) `X-Confirm-Passphrase` header, the response is `401 Unauthorized` — same convention as `confirm_passphrase` in `app/core/security.py` and `_INVALID_CREDS` in `app/api/auth.py`.
**Why:** The argument for 403 is real — the user IS authenticated (the JWT was valid), they just failed to elevate. But returning 401 keeps every "credential failed" surface in the app speaking the same dialect; mixing 401 and 403 invites client-side handling drift. Single-user app, single auth boundary — the consistency wins.
**Alternatives considered:** Return 403 for step-up failures (rejected — see above); return a distinct status like 412 Precondition Failed (rejected — non-standard for this use case; clients would have to special-case it).

## 2026-05-09 — `structlog` `cache_logger_on_first_use=False`
**Choice:** `app/core/logging.py` configures structlog with `cache_logger_on_first_use=False`. Each emit reads the current global config; existing `BoundLoggerLazyProxy` instances pick up `configure_logging()` re-runs.
**Why:** With caching ON, a logger created at module import (when conftest's `LOG_LEVEL=WARNING` is in effect) keeps its WARNING filter forever — the privacy-guard test's `LOG_LEVEL=INFO` toggle silently has no effect on application loggers, and the "no leak" assertions pass vacuously. Disabling the cache makes `configure_logging` actually reconfigure live loggers, which is what tests (and any future operator-driven runtime LOG_LEVEL change) need. Per-call overhead is unmeasurable at single-user scale.
**Alternatives considered:** Keep caching ON and patch the test-time fixture to invalidate caches (rejected — relies on structlog internals, brittle); reconfigure loggers globally on every flip (rejected — same brittleness, more code).
